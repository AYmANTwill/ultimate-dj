...
| 2026-08-27 | Recorder intégré à l'app (page **Record**) | Le recorder complet (morceaux voie B + gestes knob-par-knob voie A) devient une **page dans l'app** : `ui/recorder.py` (`RecorderPage`, groupe MIX sidebar) — bouton Démarrer/Arrêter, liste des morceaux joués en direct + gestes captés avec compteur par contrôle, sauve le document combiné dans `data/recorded_sets/`. UI thread-safe (callbacks marshalés via `after`). Build headless vérifié, sidebar MIX = Mixer·Live·Record·Setlist. |
| 2026-08-27 | ⚔ **VOIE A DÉBLOQUÉE : capture knob-par-knob PENDANT Rekordbox** | Correction de la conclusion « impasse MIDI » ci-dessous : elle reposait sur des captures vides alors que l'user était **parti de son bureau** (rien bougé), pas sur un verrou. Recherche user (git/google) → **Mixxx** : le DDJ-FLX4 est muet jusqu'à une **SysEx de réveil** `F0 00 40 05 00 00 04 05 00 50 02 F7` envoyée à sa sortie MIDI (+keep-alive 3 s), puis il reporte tout ; et `Pioneer-DDJ-FLX4.midi.xml` donne la **carte complète**. Intégré dans `midi_recorder.py` (SysEx + 30 contrôles mappés + `label()`). **Prouvé Rekordbox OUVERT, user présent : 2670 gestes / 22 contrôles tous nommés** (crossfader, volumes, EQ hi/mid/low ×2, filters…) — le FLX4 expose MIDI (MI_00) ET HID (MI_05) simultanément, on lit le MIDI pendant que Rekordbox lit le HID. **Pas besoin d'USBPcap.** `record_set.py` = recorder COMPLET : morceaux (voie B) + gestes knob-par-knob horodatés (voie A) → `data/recorded_sets/*.json`. Reste : C (MIDI virtuel pour l'agent qui joue) + Player. |
| 2026-08-27 | Agent DJ — MIDI/HID en impasse → **Recorder v1 voie B livré** | Diagnostic matériel poussé sur le DDJ-FLX4 : il n'envoie **pas de MIDI** (port `DDJ-FLX4 0` muet), il parle **HID propriétaire** (interface MI_05, usage_page 0xFF60) que **Rekordbox verrouille en exclusif** — capture MIDI ET HID = 0 message même Rekordbox ouvert. Idem PRO DJ LINK (ports 50000-2 tenus en exclusif, WinError 10013). Conclusion : sur ce setup laptop+FLX4, on ne peut pas lire les gestes knob-par-knob sans sniffer USB (USBPcap, chantier lourd = voie A, plus tard). **Décision user : voie B+C.** Voie B livrée : `engine/set_recorder.py` (`SetRecorder` sonde l'historique `master.db` en temps réel — même accès lecture-seule que le Mode Live — et log la séquence de morceaux horodatée en « document de set » JSON `data/recorded_sets/`) + `record_set.py` (runner). **Vérifié en direct : a capturé les 32 morceaux de la session Rekordbox ouverte, dans l'ordre.** Suite : C = port MIDI virtuel (loopMIDI) mappé dans Rekordbox pour que l'agent JOUE ; puis le Player rejoue un document de set ; puis l'agent autonome (scoring L1-L5). Libs MIDI/HID installées (mido/python-rtmidi/hidapi) — restent utiles pour la voie A future. |
| 2026-08-27 | 🎛️ NOUVEAU CAP : Agent DJ — Recorder MIDI (jalon 1) | Nouvelle vision majeure (user) : un agent qui mixe seul, adossé à une base de ses gestes réels. Architecture décidée : **Rekordbox n'a pas d'API de contrôle → MIDI** (capture pour enregistrer, envoi pour jouer) + PRO DJ LINK/pyrekordbox pour l'état des decks. 3 étages : Recorder → Player (rejoue un doc de set) → Agent autonome (réutilise le scoring L1-L5). Setup user : **contrôleur USB DDJ-FLX4** — confirmé exposer du **MIDI standard** (`DDJ-FLX4 0` in / `DDJ-FLX4 1` out). Jalon 1 livré : `engine/midi_recorder.py` (mido/python-rtmidi optionnels, `MidiRecorder` horodate chaque cc/note/pitch, schéma JSON `{t,kind,ch,num,val,raw}`, `probe()` apprend la carte du contrôleur) + `midi_probe.py` (runner). Vérifié sur le vrai matériel : port DDJ ouvert, capture active. **Reste (user, à son matos)** : lancer `python midi_probe.py` en bougeant les contrôles (Rekordbox fermé PUIS ouvert — teste l'accès exclusif du port) → carte MIDI exacte du FLX4 → Recorder complet corrélé track/position. |
| 2026-08-22 | Fix yt-dlp périmé (403 chez l'ami) → v1.6.1 | L'ami (julia) lance Ultimate DJ Lite, l'app tourne et résout bien le titre Spotify, mais le téléchargement YouTube échoue : `HTTP Error 403: Forbidden`. Cause confirmée : yt-dlp bundlé = **2026.03.17** (5 mois), YouTube a changé ; repro exacte du 403 en dev avec cette version. `pip install -U yt-dlp` → **2026.08.19** → le titre exact (Taylor Swift, Toy Story 5) se télécharge (2,79 Mo). Fix : pin `yt-dlp[default]>=2026.8.19` dans requirements, **bump `app/version.py` 1.6.0→1.6.1** (pour que l'updater de julia détecte), rebuild lite. Bonus : `downloader._friendly_error` transforme un 403/nsig en message actionnable (« va dans Réglages › Vérifier les mises à jour »). Leçon durable : yt-dlp se périme vite — l'updater whole-app EST le canal de rafraîchissement ; option future = yt-dlp.exe auto-`-U`. NB disque : builds jonglés (D: sature, `Music - Copie` 30 Go). |
| 2026-07-18 | Édition LITE + auto-updater GitHub livrés | Suite du chantier ami. **Édition lite** (`app/edition.py`, drapeau `ULTIMATEDJ_LITE`) : même codebase, sidebar réduite à **Download + Library + Settings** (vérifié : full inchangée `[Home,Download,Library,Analyze,Mixer,Live,Setlist,Discover,Settings]` → lite `[Download,Library,Settings]`), titre « Ultimate DJ Lite », atterrissage sur la 1re page dispo (pas de Home). **`ui/settings_lite.py`** : page minimale autonome (dossiers musique/download, creds Spotify, section mises à jour) — pas de sections IA/workers. **Auto-updater consenti** (`engine/updater.py`, choix user = GitHub Releases) : vérifie `api.github.com/repos/AYmANTwill/ultimate-dj/releases/latest`, compare à `app/version.py` (1.6.0), télécharge le zip, applique via un `.bat` qui attend la sortie du process → robocopy → relance (Windows/frozen only, no-op ailleurs). **PAS un canal caché** : rien ne s'exécute sans le clic de l'ami, source épinglée au repo owner. Build lite : `ultimate_dj_lite.spec` (+ `rthook_lite.py` qui pose le drapeau, exclut pyrekordbox/sqlcipher3) + `build_share_lite.py` → `dist/UltimateDJ-Lite`. Stdlib pur, zéro nouvelle dépendance. 61 tests (deezer/updater/edition ajoutés). |
| 2026-07-18 | Deezer comme source + chantier « app lite pour un ami » | Demande : une version **lite** (Library + Download seulement) pour un ami non-technique sur Windows 11 + Rekordbox, avec **Deezer** en plus des sources existantes, et un **canal d'update** que l'owner pousse. Deezer livré (phase 1, partagé avec l'app principale) : `engine/deezer.py` calque `spotify.py` sur l'API **publique** deezer (aucun login, aucun token, aucun flux DRM — metadata → download YouTube par recherche, identique au flux Spotify). Dispatch `download._start_download` généralisé (`_download_spotify`→`_download_from_source(resolver, …)`, agnostique), libellés + placeholder mentionnent Deezer. Testé en live (track HBFS, album Discovery 14 titres) + test offline url_id/_track_item. Restent : édition lite (drapeau sidebar), auto-updater GitHub Releases (choix user), build lite séparé. |
